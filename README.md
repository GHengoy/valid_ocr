...
```bash
cd ~/valid_ocr
bash setup.sh          # 의존성 설치 + (선택)엔진 빌드 + 바탕화면 아이콘 + 자동실행 등록
./start.sh             # 실행
```
...
옵션: `bash setup.sh --build-trt`(엔진까지 자동) / `--no-trt`(엔진 건너뜀)
...
| 동작 | 방법 |
|---|---|
| 실행 | 바탕화면 **일부인 검증 시스템** 아이콘 또는 `./start.sh` |
...
`./start.sh` 는 터미널에서 실행 시 스캔할 때마다 인식 로그를 출력합니다:
```
...
onnx/
  ...
  korean_rec.onnx / korean_det.onnx / korean_dict.txt
  korean_rec.trt    (빌드 산출물)
setup.sh            전체 설치
start.sh            실행 런처
...
정확도는 `python3 verify_ocr.py` 로 기계별 재측정할 수 있습니다.
